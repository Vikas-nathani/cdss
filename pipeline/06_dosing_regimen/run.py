    ...
    while True:
        ...
        try:
            async with pool.acquire() as conn:
                raw_rows = await conn.fetch(FETCH_SQL, batch_limit, current_offset)
        except Exception as e:
        ...
        # Advance offset by the number of raw rows returned so next fetch
        # moves past this window regardless of how many workers have committed.
        ...
        logger.info(f"Producer: fetched batch of {len(rows)} (raw: {len(raw_rows)}, offset now: {current_offset})")
        ...
        if len(raw_rows) < batch_limit:
            break  # last partial batch — truly exhausted
