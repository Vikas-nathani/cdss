...
CHECKPOINT_FILE  = Path("~/cdss/retry_zero_rows_v2_checkpoint.json").expanduser()
LOG_FILE         = Path("~/cdss/logs/retry_zero_rows_v2.log").expanduser()
DEAD_LETTER_FILE = Path("~/cdss/logs/retry_zero_rows_v2_failed.log").expanduser()
RESULT_LOG_FILE  = Path("~/cdss/logs/retry_zero_rows_v2_results.log").expanduser()
RESPONSE_CACHE_PATH = str(Path("~/cdss/logs/retry_zero_rows_v2_responses.jsonl").expanduser())

LOG_EVERY     = 500    # log progress every N drugs
MAX_RETRIES   = 2
DRY_RUN_LIMIT = 20     # set to None for full run
CHUNK_SIZE    = 2000   # max chars per text chunk sent to LLM
...
TOTAL_DRUGS      = 289       # remaining ZERO_ROWS drugs after v1 run
    ...
    while True:
        ...
        try:
            ...
            if not rows:
                ...
                category = zero_rows_categories.get(str(master_linkage_id))
                try:
                    async with pool.acquire() as conn:
                        if category == 'AMBIGUOUS':
                            # Gave it one retry chance — permanently close out
                            await conn.execute("""
                                UPDATE drugdb.failed_drugs
                                SET failure_reason = 'NO_DOSING_INFO',
                                    failed_at = NOW()
                                WHERE master_linkage_id = $1
                            """, master_linkage_id)
                            logger.info(
                                f"Parser: AMBIGUOUS drug {generic_name} ({master_linkage_id}) "
                                f"still zero rows — marked NO_DOSING_INFO"
                            )
                        else:
                            # BAD_LLM_RESPONSE or unknown — keep as ZERO_ROWS for potential retry
                            await conn.execute("""
                                INSERT INTO drugdb.failed_drugs
                                    (master_linkage_id, generic_name, failure_reason)
                ...
                try:
                    async with pool.acquire() as conn:
                        await conn.execute("""
                            ...
                            WHERE master_linkage_id = $1
                            AND failure_reason = 'ZERO_ROWS'
                        """, master_linkage_id)
                except Exception as fe:
                    logger.warning(f"Parser: failed_drugs ZERO_ROWS delete error: {fe}")
...
async def main(dry_run: bool = False) -> None:
    ...
    import csv as _csv
    zero_rows_categories = {}
    try:
        with open(os.path.expanduser('~/cdss/zero_rows_analysis.csv')) as f:
            reader = _csv.DictReader(f)
            for row in reader:
                zero_rows_categories[row['master_linkage_id']] = row['category']
        logger.info(f"Loaded zero_rows_analysis.csv: {len(zero_rows_categories)} entries")
    except Exception as e:
        logger.warning(f"Could not load zero_rows_analysis.csv: {e}")
    ...
    try:
        ...
        await asyncio.gather(
            ...
            *workers,
            parser(pool, result_q, write_q, zero_rows_categories),
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="retry_zero_rows_v2 pipeline — chunked text processing")
    ...
    arg_parser.add_argument(
        "--full-run",
        action="store_true",
        help="Process all 289 remaining ZERO_ROWS records",
    )
    args = arg_parser.parse_args()

    if not args.dry_run and not args.full_run:
        print("ERROR: Must specify --dry-run or --full-run")
        print("  python retry_zero_rows_v2.py --dry-run   # test 20 records")
        print("  python retry_zero_rows_v2.py --full-run  # all 289 ZERO_ROWS records")
        sys.exit(1)
