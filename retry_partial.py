#!/usr/bin/env python3
"""
Phase 3 Step 3.2: Dosing Regimen Extraction Pipeline — PARTIAL retry

Producer/Consumer pattern with 4 async stages running simultaneously.
Nothing is idle: producer feeds fetch_q, 10 API workers drain it into
...
Stages:
  ...
  2. api_worker  — calls DeepSeek API (10 concurrent), enqueues parsed JSON
...
CHECKPOINT_FILE  = Path("~/cdss/retry_partial_checkpoint.json").expanduser()
LOG_FILE         = Path("~/cdss/logs/retry_partial.log").expanduser()
DEAD_LETTER_FILE = Path("~/cdss/logs/retry_partial_failed.log").expanduser()
RESULT_LOG_FILE  = Path("~/cdss/logs/retry_partial_results.log").expanduser()
RESPONSE_CACHE_PATH = str(Path("~/cdss/logs/retry_partial_responses.jsonl").expanduser())

LOG_EVERY     = 500    # log progress every N drugs
MAX_RETRIES   = 2
DRY_RUN_LIMIT = 20     # set to None for full run
CHUNK_SIZE    = 4000   # max chars per text chunk sent to LLM
...
TOTAL_DRUGS      = 730       # PARTIAL drugs remaining after first partial run
...
def chunk_and_build_payloads(drug_name: str, openfda_text: str, dailymed_text: str) -> list:
    """
    Split long dosage text into CHUNK_SIZE chunks and build one payload per chunk.
    Each chunk is labelled with its part number so the LLM knows it is a section.
    Returns a list of payloads — one per chunk.
    If total text is under CHUNK_SIZE, returns a single payload (no chunking needed).
    """
    full_text = ""
    if openfda_text and openfda_text.strip():
        full_text += f"[OpenFDA]\n{openfda_text.strip()}"
    if dailymed_text and dailymed_text.strip():
        if full_text:
            full_text += "\n\n"
        full_text += f"[DailyMed]\n{dailymed_text.strip()}"

    if not full_text:
        return []

    if len(full_text) <= CHUNK_SIZE:
        return [build_payload(drug_name, openfda_text, dailymed_text)]
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": SYSTEM_PROMPT},
                {
                    ...
                    "content": (
                        f"Drug: {drug_name}\n"
                        f"(Part {i+1} of {len(chunks)} — extract ALL dosing regimens from this section)\n\n"
                        f"{chunk}"
                    ),
                },
            ],
...
FETCH_SQL = """
    ...
    INNER JOIN drugdb.failed_drugs f ON f.master_linkage_id = dml.master_linkage_id
    WHERE f.failure_reason = 'PARTIAL'
...
async def producer(
    ...
    that receives None passes it forward so all workers stop cleanly.
    """
...
async def api_worker(
    ...
    while True:
        ...
        try:
            ...
            payloads = chunk_and_build_payloads(generic_name, openfda_text, dailymed_text)

            if not payloads:
                logger.warning(f"Worker {worker_id}: no text for {master_linkage_id} ({generic_name}) — skipping")
                continue
                    ...
                    try:
                        async with session.post(DEEPSEEK_URL, json=payload, headers=headers) as resp:
                            if resp.status in (429, 500, 502, 503):
                                ...
                                logger.warning(
                                    f"Worker {worker_id}: {resp.status} for "
                                    f"{master_linkage_id} chunk {chunk_idx+1}/{len(payloads)}, "
                                    f"retry {attempt+1}/{MAX_RETRIES}, backoff {wait_secs:.1f}s"
                                )
                            ...
                            if resp.status != 200:
                                ...
                                logger.error(
                                    ...
                                    f"{master_linkage_id} chunk {chunk_idx+1}: {body[:300]}"
                                )
                                break
                        ...
                        logger.warning(
                            ...
                            f"chunk {chunk_idx+1} (attempt {attempt+1}/{MAX_RETRIES}): {e}"
                        )
                        await asyncio.sleep(wait_secs)

                if response_data is None:
                    logger.error(
                        f"Worker {worker_id}: all retries exhausted for "
                        f"{master_linkage_id} chunk {chunk_idx+1}/{len(payloads)}"
                    )
                    any_failed = True
            ...
            if any_failed or not all_responses:
                continue

            await result_q.put((master_linkage_id, generic_name, all_responses))
...
async def parser(
    ...
    while True:
        ...
        try:
            ...
            master_linkage_id, generic_name, response_data_list = item
            ...
            # Deduplicate rows by semantic key before inserting
            seen_ids = set()
            unique_rows = []
            for row in rows:
                pop = row.get("population") or {}
                dedup_key = "|".join([
                    str(row.get("indication") or ""),
                    str(pop.get("age_group") or ""),
                    str(pop.get("renal_function") or ""),
                    str(pop.get("hepatic_function") or ""),
                    str(row.get("route") or ""),
                    str(row.get("frequency") or ""),
                    str(row.get("dose_amount") or ""),
                ])
                if dedup_key not in seen_ids:
                    seen_ids.add(dedup_key)
                    unique_rows.append(row)
            logger.info(f"Parser: {master_linkage_id} — {len(all_rows)} raw rows → {len(unique_rows)} after dedup")
            rows = unique_rows
                ...
                try:
                    async with pool.acquire() as conn:
                        await conn.execute("""
                            ...
                            ON CONFLICT (master_linkage_id) DO UPDATE
                            SET failure_reason = 'ZERO_ROWS',
                                attempt_count = failed_drugs.attempt_count + 1,
                                failed_at = NOW()
                        """, master_linkage_id, generic_name)
                    logger.warning(
                        f"Parser: {master_linkage_id} ({generic_name}) returned zero rows on partial retry — marked ZERO_ROWS"
                    )
                except Exception as fe:
                    logger.warning(f"Parser: failed_drugs ZERO_ROWS insert error: {fe}")
            ...
            if is_partial:
                try:
                    async with pool.acquire() as conn:
                        ...
                        """, master_linkage_id, generic_name)
                    logger.warning(
                        f"Parser: {master_linkage_id} ({generic_name}) still PARTIAL after chunked retry — kept in failed_drugs"
                    )
                except Exception as fe:
                    logger.warning(f"Parser: failed_drugs PARTIAL update error: {fe}")
            ...
            if not is_partial:
                try:
                    async with pool.acquire() as conn:
                        await conn.execute("""
                            DELETE FROM drugdb.failed_drugs
                            WHERE master_linkage_id = $1
                            AND failure_reason = 'PARTIAL'
                        """, master_linkage_id)
                    logger.info(f"Parser: removed {master_linkage_id} from failed_drugs (PARTIAL resolved)")
                except Exception as fe:
                    logger.warning(f"Parser: failed_drugs PARTIAL delete error: {fe}")
...
async def main(dry_run: bool = False) -> None:
    ...
    and a shared stats dict. Runs producer + 10 api_workers + parser +
    ...
    logger.info(
        f"=== retry_partial pipeline STARTED === "
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="retry_partial pipeline — chunked text processing for PARTIAL drugs")
    ...
    arg_parser.add_argument(
        "--full-run",
        action="store_true",
        help="Process all 1069 remaining PARTIAL records",
    )
    args = arg_parser.parse_args()

    if not args.dry_run and not args.full_run:
        print("ERROR: Must specify --dry-run or --full-run")
        print("  python retry_partial.py --dry-run   # test 20 records")
        print("  python retry_partial.py --full-run  # all 1069 PARTIAL records")
        sys.exit(1)
