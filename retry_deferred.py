...
CHECKPOINT_FILE  = Path("~/cdss/retry_deferred_checkpoint.json").expanduser()
LOG_FILE         = Path("~/cdss/logs/retry_deferred.log").expanduser()
DEAD_LETTER_FILE = Path("~/cdss/logs/retry_deferred_failed.log").expanduser()
RESULT_LOG_FILE  = Path("~/cdss/logs/retry_deferred_results.log").expanduser()
RESPONSE_CACHE_PATH = str(Path("~/cdss/logs/retry_deferred_responses.jsonl").expanduser())
...
TOTAL_DRUGS      = 793       # DEFERRED drugs only
...
FETCH_SQL = """
    ...
    WHERE f.failure_reason = 'DEFERRED'
                ...
                try:
                    async with pool.acquire() as conn:
                        await conn.execute("""
                            ...
                            WHERE master_linkage_id = $1
                            AND failure_reason = 'DEFERRED'
                        """, master_linkage_id)
                except Exception as fe:
                    logger.warning(f"Parser: failed_drugs DEFERRED delete error: {fe}")
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="retry_deferred pipeline")
    ...
    arg_parser.add_argument(
        "--full-run",
        action="store_true",
        help="Process all 793 DEFERRED records",
    )
    args = arg_parser.parse_args()

    if not args.dry_run and not args.full_run:
        print("ERROR: Must specify --dry-run or --full-run")
        print("  python retry_deferred.py --dry-run   # test records")
        print("  python retry_deferred.py --full-run  # all 793 DEFERRED records")
        sys.exit(1)
