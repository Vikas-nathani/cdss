...
CHECKPOINT_FILE  = Path("~/cdss/retry_zero_rows_checkpoint.json").expanduser()
LOG_FILE         = Path("~/cdss/logs/retry_zero_rows.log").expanduser()
DEAD_LETTER_FILE = Path("~/cdss/logs/retry_zero_rows_failed.log").expanduser()
RESULT_LOG_FILE  = Path("~/cdss/logs/retry_zero_rows_results.log").expanduser()
RESPONSE_CACHE_PATH = str(Path("~/cdss/logs/retry_zero_rows_responses.jsonl").expanduser())

LOG_EVERY     = 500    # log progress every N drugs
MAX_RETRIES   = 2
DRY_RUN_LIMIT = 100    # set to None for full run
...
TOTAL_DRUGS      = 349       # BAD_LLM_RESPONSE + AMBIGUOUS ZERO_ROWS drugs
...
FETCH_SQL = """
    ...
    AND dml.master_linkage_id NOT IN (
        ...
        JOIN drugdb.drug d2 ON d2.formulation_id = dr.formulation_id
        WHERE d2.master_linkage_id IS NOT NULL
    )
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="retry_zero_rows pipeline")
    ...
    arg_parser.add_argument(
        "--full-run",
        action="store_true",
        help="Process all 349 ZERO_ROWS records",
    )
    args = arg_parser.parse_args()

    if not args.dry_run and not args.full_run:
        print("ERROR: Must specify --dry-run or --full-run")
        print("  python retry_zero_rows.py --dry-run   # test records")
        print("  python retry_zero_rows.py --full-run  # all 349 ZERO_ROWS records")
        sys.exit(1)
